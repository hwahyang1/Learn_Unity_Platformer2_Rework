...
namespace _20220531_Platform2Rework.GameScene.Props
{
	...
	public class Chest : EventProp
	{
		...
		public override void OnInteract()
		{
			if (isUsed)
			{
				script.PrintScript(new string[] { "상자 안에 다른 물건은 없는 듯 하다..." });
			}
			else
			{
				...
				else
				{
					...
					if (response == 0)
					{
						script.PrintScript(new string[] { "상자 안에서 " + getItemNames(giveItem) + "(을)를 획득했다!" });
						isUsed = true;
					}
					else
					{
						script.PrintScript(new string[] { "상자 안에 무언가가 있다!\n하지만 가방에 충분한 공간이 없는 듯 하다..." });
					}
				}
			}
		}
